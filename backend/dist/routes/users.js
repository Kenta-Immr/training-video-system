...
router.delete('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        // 関連する視聴ログも削除
        await index_1.prisma.viewingLog.deleteMany({
            where: { userId }
        });
...
// パスワードリセット（管理者のみ）
router.post('/:id/reset-password', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const userId = parseInt(req.params.id);
...
// 一括ユーザー作成（管理者のみ）
router.post('/bulk-create', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
            ...
            try {
                ...
                if (groupName && groupName.trim()) {
                    ...
                    if (groupCache.has(trimmedGroupName)) {
                        ...
                        // 既存グループを検索
                        let group = await index_1.prisma.group.findFirst({
                        ...
                        if (!group) {
                            try {
                                ...
                                let counter = 1;
                                while (await index_1.prisma.group.findUnique({ where: { code: finalGroupCode } })) {
                                    finalGroupCode = `${groupCode}_${counter}`;
                                    counter++;
                                }
                                group = await index_1.prisma.group.create({
                                    data: {
...
// 未ログインユーザー一覧取得（管理者のみ）
router.get('/reports/first-login-pending', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const users = await index_1.prisma.user.findMany({
            where: {
                isFirstLogin: true,
