...
router.put('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const group = await index_1.prisma.group.update({
            where: { id: groupId },
            data: {
                name,
                code,
...
router.delete('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        // グループを削除
        await index_1.prisma.group.delete({
            where: { id: groupId }
        });
        res.status(204).send();
    }
...
// ユーザーをグループに追加（管理者のみ）
router.post('/:id/users', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
...
// ユーザーをグループから削除（管理者のみ）
router.delete('/:id/users', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
        const { userIds } = req.body;
        if (!Array.isArray(userIds) || userIds.length === 0) {
...
// グループのコース権限取得（管理者のみ）
router.get('/:id/courses', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
...
// グループにコース権限を追加（管理者のみ）
router.post('/:id/courses', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
        ...
        // 既存の権限をチェックして重複を避ける
        const existingPermissions = await index_1.prisma.groupCourse.findMany({
            where: {
                groupId,
        ...
        if (newCourseIds.length > 0) {
            await index_1.prisma.groupCourse.createMany({
...
router.delete('/:id/courses', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
        const { courseIds } = req.body;
        if (!Array.isArray(courseIds) || courseIds.length === 0) {
        ...
        await index_1.prisma.groupCourse.deleteMany({
            where: {
                groupId,
                courseId: { in: courseIds }
            }
...
router.get('/:id/progress/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const groupId = parseInt(req.params.id);
        ...
        // 対象ユーザーの詳細進捗を取得
        const userProgress = await index_1.prisma.user.findUnique({
