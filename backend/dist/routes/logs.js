...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // 既存のログを確認
        const existingLog = await index_1.prisma.viewingLog.findUnique({
            where: {
                userId_videoId: {
                    userId,
        ...
        if (existingLog) {
            // 既存ログの更新（より多い視聴時間で更新）
            viewingLog = await index_1.prisma.viewingLog.update({
                where: {
                    userId_videoId: {
                        userId,
                        videoId: parseInt(videoId)
            ...
            // 新規ログ作成
            viewingLog = await index_1.prisma.viewingLog.create({
                data: {
                    userId,
                    videoId: parseInt(videoId),
...
// ユーザー別視聴ログ取得（管理者のみ）
router.get('/users/:userId', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        const logs = await index_1.prisma.viewingLog.findMany({
            where: { userId },
            include: {
                video: {
                    include: {
...
// 動画別視聴ログ取得（管理者のみ）
router.get('/videos/:videoId', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.get('/stats', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        // ユーザー別統計
        const userStats = await index_1.prisma.user.findMany({
            where: { role: 'USER' },
            select: {
                id: true,
                name: true,
        ...
        // 全動画数を取得
        const totalVideos = await index_1.prisma.video.count();
        // ユーザー別進捗率を計算
...
// 自分の視聴ログ取得（受講者）
router.get('/my-logs', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const logs = await index_1.prisma.viewingLog.findMany({
            where: { userId },
