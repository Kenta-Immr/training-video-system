...
async function ensureDataInitialized() {
  // ホットリロード時: 既存データがあれば初期化済みとみなす
  if (memoryStorage.initialized || 
      (memoryStorage.users && memoryStorage.courses && memoryStorage.groups)) {
    console.log('データ初期化スキップ: 既存データまたは初期化済み')
...
// ハイブリッドデータ読み込み（メモリ優先、ファイルバックアップ）
...
function getCourses() {
  try {
    ...
    // ホットリロード対応: 既存データがあるかチェック
    if (memoryStorage.courses && memoryStorage.courses.courses && 
        typeof memoryStorage.courses.courses === 'object') {
      const coursesList = Object.values(memoryStorage.courses.courses)
      console.log('getCourses: メモリから直接取得 (ホットリロード対応)', coursesList.length, '件')
      return coursesList
    }
    
    // データが無い場合の初期化
    if (!memoryStorage.initialized && !memoryStorage.initializationPromise) {
      console.log('getCourses: 緊急初期化を実行')
      loadCoursesData()
    }
...
function getUsers() {
  // ホットリロード対応: 既存データがあるかチェック
  if (memoryStorage.users && memoryStorage.users.users && 
      typeof memoryStorage.users.users === 'object') {
    console.log('getUsers: メモリから直接取得 (ホットリロード対応)', Object.keys(memoryStorage.users.users).length, '件')
    return Object.values(memoryStorage.users.users)
  }
  
  // データが無い場合の初期化
  if (!memoryStorage.initialized && !memoryStorage.initializationPromise) {
    console.log('getUsers: 緊急初期化を実行')
    loadUsersData()
  }
...
function getGroups() {
  // ホットリロード対応: 既存データがあるかチェック
  if (memoryStorage.groups && memoryStorage.groups.groups && 
      typeof memoryStorage.groups.groups === 'object') {
    console.log('getGroups: メモリから直接取得 (ホットリロード対応)', Object.keys(memoryStorage.groups.groups).length, '件')
    return Object.values(memoryStorage.groups.groups)
  }
  
  // データが無い場合の初期化
  if (!memoryStorage.initialized && !memoryStorage.initializationPromise) {
    console.log('getGroups: 緊急初期化を実行')
    loadGroupsData()
  }
