...
function loadCoursesData() {
  try {
    if (fs.existsSync(COURSES_FILE)) {
      ...
      // データ構造の検証
      if (!parsed.courses) {
        console.log('コースデータ構造が不正です。デフォルトデータで初期化します。')
      ...
      console.log('コースファイルが存在しません。新規作成します。')
      saveCoursesData(DEFAULT_COURSES_DATA)
      return DEFAULT_COURSES_DATA
    }
  } catch (error) {
    console.error('コースデータの読み込みエラー:', error)
    console.log('デフォルトデータで復旧します。')
    saveCoursesData(DEFAULT_COURSES_DATA)
    return DEFAULT_COURSES_DATA
  }
}
...
function getCourses() {
  try {
    ...
    console.log('getCourses: データ取得完了', Object.keys(data.courses || {}).length, '件')
