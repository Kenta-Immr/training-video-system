  ...
  try {
    ...
    if (useKV) {
      ...
      try {
        ...
        console.warn('KV保存中にエラーが発生しましたが、メモリには保存済み:', error.message)
      }
    } else if (isVercel) {
      // KVが利用できない場合でも本番環境では警告を出してメモリ保存を継続
      console.warn('本番環境ですがKVが利用できません。メモリ内のみでデータを保持します。')
      console.log('Vercel環境: メモリストレージのみでデータ保存を継続')
    }
...
async function createCourseAsync(courseData) {
  try {
    console.log('createCourseAsync呼び出し:', courseData)
    ...
    // 本番環境では非同期保存でKVに確実に保存
    if (isVercel) {
      try {
        await saveCoursesData(data)
        console.log('コースデータ保存成功')
      } catch (saveError) {
        console.error('KV保存エラーですが、メモリには保存済み:', saveError)
        // KV保存失敗でもメモリには保存されているので継続
      }
    } else {
      saveCoursesDataSync(data)
    }
    ...
    console.error('createCourseAsync内エラー:', error)
    throw error
  }
}
...
async function createUserAsync(userData) {
  try {
    console.log('createUserAsync呼び出し:', userData)
    ...
    if (isVercel) {
      try {
        ...
        console.error('KVユーザー保存エラーですが、メモリには保存済み:', saveError)
      }
    } else {
      saveUsersData(data)
    }
    ...
    console.error('createUserAsync内エラー:', error)
    throw error
  }
}
...
async function createGroupAsync(groupData) {
  try {
    console.log('createGroupAsync呼び出し:', groupData)
    ...
    if (isVercel) {
      try {
        await saveGroupsDataAsync(data)
        console.log('グループデータ保存成功')
      } catch (saveError) {
        console.error('KVグループ保存エラーですが、メモリには保存済み:', saveError)
      }
    } else {
      saveGroupsData(data)
    }
    ...
    console.error('createGroupAsync内エラー:', error)
    throw error
  }
}
