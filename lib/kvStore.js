...
let kv = null
try {
  // @vercel/kvは本番環境でのみ利用可能
  if (process.env.NODE_ENV === 'production' && process.env.KV_REST_API_URL) {
    // 動的require（Webpack警告回避）
    const vercelKv = eval('require("@vercel/kv")')
    kv = vercelKv.kv
  }
} catch (error) {
  console.log('KV module not available:', error.message)
}
...
function isKVAvailable() {
  return isProduction && hasKVConfig && kv !== null
}
